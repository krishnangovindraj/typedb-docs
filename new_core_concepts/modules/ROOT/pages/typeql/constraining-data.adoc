...
These constraints can restrict the cardinality of an interface, or the range of values of an attribute-type.

These constraints are written using *annotations* in TypeQL. Annotations are written with `@` and apply to schema declarations. Note that there are other types of annotations besides data constraint annotations as well.
...
Similarly, inherited cardinality constraints on `plays` & `owns` can be specialized. e.g.:
[,typeql]
----
