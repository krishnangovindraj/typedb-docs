...
Most databases just have value types and no additional layer wrapping them into new types. The advantage is that there may be many different meanings of a value type in different contexts. For example, sometimes an `integer` represents an `age`, but in other cases it could be a `salary`. Attribute types allow us to convey distinct meaning and intent to users and the database, and utilize subtyping to build nuanced hierarchies.

== Owning attributes & playing roles

Entity & relation types may be defined to "own" attribute types.
Unlike attributes, which are identified by their values, there is no meaningful way to reference an entity. Hence, entities (or relations) are referenced through their attributes.
...
The same principle applies for `owns`. This compositional approach of playing and owning roles and attribute types can also be viewed as a kind of structural polymorphism.

=== Subtyping
TypeDB follows a single-inheritance *subtyping* system which allows types to specialize an existing type.
The subtype "inherits" all owns or plays implementations from its supertype. Relation subtypes also inherit role declarations from their supertypes.

As with any subtyping, this means an instance of a subtype may be used anywhere an instance of its super-type is expected.
...
A subtype also inherits any roles defined by its supertype.
It may optionally "specialize" an inherited interface by defining a subtype of that interface.
...
Specialization also disallows data from expressing the parent role in child relation instances. In other words, there will be no data where players play the role `interaction:content` in `content-engagement` relations.

__Advanced explanation:__
...
define
  ...
  relation content-engagment @abstract,
    sub interaction,
